...
static void draw_gauge(lv_obj_t *canvas, const struct status_state *state) {
    ...
    lv_canvas_draw_img(canvas, 16, 44 + BUFFER_OFFSET_BOTTOM, &gauge, &img_dsc);
}
...
static void draw_needle(lv_obj_t *canvas, const struct status_state *state) {
    ...
    int centerX = 33;
    int centerY = 67 + BUFFER_OFFSET_BOTTOM;
    int offset = 13;
...
static void draw_grid(lv_obj_t *canvas) {
    ...
    lv_canvas_draw_img(canvas, 0, 65 + BUFFER_OFFSET_BOTTOM, &grid, &img_dsc);
}
...
static void draw_graph(lv_obj_t *canvas, const struct status_state *state) {
    ...
    int baselineY = 97 + BUFFER_OFFSET_BOTTOM;
...
static void draw_label(lv_obj_t *canvas, const struct status_state *state) {
    ...
    lv_canvas_draw_text(canvas, 0, 101 + BUFFER_OFFSET_BOTTOM, 25, &label_left_dsc, "WPM");
    ...
    lv_canvas_draw_text(canvas, 26, 101 + BUFFER_OFFSET_BOTTOM, 42, &label_dsc_wpm, wpm_text);
}
